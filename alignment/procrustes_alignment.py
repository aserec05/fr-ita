...
from fr_ita.utils import load_embeddings, save_embeddings
...
def reduce_all_vectors(fr_vectors, it_vectors, max_dim=100):
    ...
    """
    print(f"Reducing all vectors from {next(iter(fr_vectors.values())).shape[0]} to {max_dim}d...")
    
    # stack all vectors together to fit SVD
    ...
    # transform each language separately
...
if __name__ == "__main__":
    ...
    # for glove (pre-trained 300d), keep full dimension for better alignment
    max_dim = 300 if embedding_type == 'glove' else 100
    
    # reduce if needed (fit on full corpus, not just pairs)
